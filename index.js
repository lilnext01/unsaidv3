...
const { unsaidmenu } = require('./src/unsaidmenu')
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOlá!\n*Você não está registrado no banco de dados, registre-se por favor.* \n\nComando: ${prefix}registrar nome|idade\nExemplo : ${prefix}registrar Unsaid|18`,
				}
			}
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '555197195835@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
				break
				case 'unsaidmenu':
					client.sendMessage(from, unsaidmenu(prefix, sender), text, {quoted: mek})
							break
							case 'menuadmin':
  ...
  client.updatePresence(from, Presence.composing) 
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite Unsaid|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo Unsaid BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					teks = `❏ *Title* : ${anu.title}\n❏ *Filesize* : ${anu.filesize}\n\nTEspere um minuto,o áudio está sendo enviado novamente ... `
	...
	case 'kbbi':
            client.updatePresence(from, Presence.composing) 
                if (args.length < 1) return reply(`Digite as perguntas\Exemplo: ${prefix}kbbi oi`)
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Esta é a lista de números bloqueados pelo Bot:\n'
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘UnsaidBOT 〙', members_id, true)
					break
                case 'marcar2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘UnsaidBOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'marcar4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Utilize este comando com o número completo (com o DDD), sem espaços, pontos ou traços. ')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Falha ao adicionar, talvez o número seja privado')
					}
					break
				...
				case 'remover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Foi de beise🤝: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					setTimeout( () => {
					client.sendMessage(from, '_Faltam 10 segundos_...  ', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf Unsaid|BOT`)
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}phlogo |Unsaid|BOT`)
					reply(mess.wait)
            ...
            break
					...
					case 'nsfwbobs': 
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tá ai os peitos que vc queria\npunhetero de merda'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				...
				case 'nsfwass':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tá ai a bunda que vc queria'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwfeets':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta ai, mas enfia sapoha no cu❤'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌') 
						}
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo UnsaidBOT`)
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo2 UnsaidBOT`)
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\nContoh: ${prefix}lionlogo Unsaid|BOT`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}shadow UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}burnpaper UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}coffee UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}lovepaper UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodblock UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}qowheart UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}mutgrass UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}undergocean UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodenboards UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}wolfmetal UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}metalictglow UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case '8bit':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}8bit Unsaid|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
				...
				case 'clone':
					...
					try {
						...
						reply('Falhou, por que possivelmente este perfil é privado')
					}
					break
                                /*case 'magernulis':
                                        var gh = body.slice(11)
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}pubglogo Unsaid|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Sei...'})
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}harrypotter UnsaidBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
